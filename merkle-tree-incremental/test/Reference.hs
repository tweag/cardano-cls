...
{- | Reference implementation based on the merkle-tree package.
The main difference is that hashes are not converted to hex strings,
but kept as raw bytes.
-}
module Reference (
  MerkleTree (..),
  MerkleRoot (..),
...
mtHash :: MerkleTree a -> Digest SHA3_256
mtHash MerkleEmpty = merkleHash ""
mtHash (MerkleTree _ x) = getMerkleRoot $ mRoot x
...
mkRootHash (MerkleRoot l) (MerkleRoot r) =
  MerkleRoot $
    merkleHash $
      mconcat
...
merkleProof (MerkleTree _ rootNode) leafRoot = MerkleProof $ constructPath [] rootNode
 where
  ...
  constructPath pElems (MerkleLeaf leafRoot' _)
    ...
    | otherwise = []
  constructPath pElems (MerkleBranch _bRoot ln rn) = lPath ++ rPath
   where
...
> Rounds              : 24
-}
sha256 :: BS.ByteString -> Digest SHA3_256
sha256 x = hash x :: Digest SHA3_256

-- | Hash function to use for merkle tree
merkleHash :: BS.ByteString -> Digest SHA3_256
merkleHash = sha256
